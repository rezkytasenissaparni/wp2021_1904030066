...
{
    global $conn;
    mysqli_query($conn, "DELETE FROM calon_mhs WHERE id=$id") or
...
{
    global $conn;
    mysqli_query($conn, "DELETE FROM tabel_dosen WHERE nidn=$ID") or
        die(mysqli_error($conn));
    return mysqli_affected_rows($conn);
}
function edit($data)
...
{
    global $conn;
    $query = "SELECT * FROM tabel_dosen WHERE nama LIKE'%$kata_kunci%'";
    $result = mysqli_query($conn, $query);
